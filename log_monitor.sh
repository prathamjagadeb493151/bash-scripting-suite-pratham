...
scan_failed_logins() {
  ...
  if [[ -f /var/log/auth.log ]]; then
    sudo grep -i "Failed password" /var/log/auth.log | tail -n 50 | while read -r l; do
      log WARN "AUTH: $l"
    done
  elif command -v journalctl >/dev/null 2>&1; then
    sudo journalctl -u ssh -S -24h -p warning --no-pager | tail -n 50 | while read -r l; do
      log WARN "AUTH: $l"
    done
  else
...
scan_top_cpu() {
  ...
  ps -eo pid,ppid,cmd,%mem,%cpu --sort=-%cpu | head -n $((TOP_CPU_N + 1)) | while read -r l; do
    log INFO "CPU: $l"
  done
}
